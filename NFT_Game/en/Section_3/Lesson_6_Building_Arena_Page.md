...
This should all look pretty familiar to you! Our contract will return `newBossHp` and `newPlayerHp` which we will then use to update the state of both our boss and character NFT (or just the boss if the player isn't ours). This part may look a bit funky, so let's dive into this a bit:
