...
Let's start by heading to this `Components/SelectCharacter/index.jsx` . Just like in `App.js` , we are going to want to hold state to know when we are the minting phase or not. Start by adding a new state property like so:
