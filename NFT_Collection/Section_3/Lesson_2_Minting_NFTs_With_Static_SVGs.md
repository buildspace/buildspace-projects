...
So for example, mines looks like this:
...
We turned our SVG code into a nice string :). base64 is basically an accepted standard for encoding data into a string. So when we say `data:image/svg+xml;base64` it's basically saying, "Hey, I'm about to give you base64 encoded data pls process it an SVG, ty!".
...
🚨Progress report.
------------------------
If you got one, send a screenshot in #progress of your fancy NFT on OpenSea in #progress :).