...
Finally, let's deploy our updated contract, mint the NFT, and make sure it works properly on Pixxiti! Deploy using the same command. I changed my deploy script a little to only mint one NFT instead of two, feel free to do the same!

```bash
npx hardhat run scripts/deploy.js --network goerli
```

Then same as before, wait a minute or two, take the contract address, search it on [https://goerli.pixxiti.com](https://goerli.pixxiti.com) and you should see your NFT there :). Again, don't click "Enter" when searching -- you need to actually click the collection when it pops up in the search bar.

Note: Remember to use `https://goerli.pixxiti.com/nfts/INSERT_DEPLOY_CONTRACT_ADDRESS_HERE/TOKEN_ID`. 

![Untitled](https://i.imgur.com/Z2mKTpK.png)

## 🚨Progress report

If you get a fancy NFT, make sure to send a screenshot of it on Pixxiti in the `#progress` channel in Discord! 
