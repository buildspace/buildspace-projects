...
------------------
If you select the <b>Contract</b> tab in Etherscan, you will notice a long list of text characters that starts from `0x608060405234801...` Hmm.. what could that be 🤔 ?


![image](https://user-images.githubusercontent.com/60590919/139609052-f4bba83c-f224-44b1-be74-de8eaf31b403.png)

It turns out that this long gibberish looking group of characters is actually the bytecodes of the contract which you have deployed! Bytecodes represents a series of opcodes in the EVM that will perform instructions for us onchain. The logic which you have written in Solidity will compile down to bytecodes which the EVM will understand. More about EVM and opcodes [here](https://ethervm.io/).

However bytecodes aren't really readible but Etherscan has the magic to 'convert' them to something more user friendly. For that magic to work, Etherscan will need your help.

Notice that there is a prompt that requests us to <b>Verify and Publish</b> our contract source code. If we follow the link, we are required to manually select our contract settings and paste our code to publish our source code.
...
Not quite done yet. Those with keen eyes will notice that Etherscan requires an ApiKey. 

If you don't have an account already, head to etherscan.io to create a free user account. After which head to your profile settings and under `API-KEYs` create a new apikey 

![image](https://user-images.githubusercontent.com/60590919/139610459-b590bbc1-0d4e-4e78-920b-c45e61bf2d7e.png)

After which, add your api key to the etherscan apikey property in our `hardhat.config.js` similarly we will be using `.env` later on for this.
...
Read contract doesn't change the state of the chain and so its free to use. Write contract however do, and so when interacting with it, you will need to pay gas. But its on the Rinkeby test net, so feel free to request more free Rinkeby Test Eth from the Rinkeby faucet so you have enough to play with.
